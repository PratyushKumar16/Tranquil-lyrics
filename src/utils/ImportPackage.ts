...
const LoadPackage = async (importUrl: PackageUrl): Promise<Package | Error | undefined> => {
  try {
    ...
    throw new Error(`TranquilLyrics [LoadPackage] ${error?.message ?? "An Error Occured"}`);
  }
};
...
export const RetrievePackage = async (
  ...
  try {
    ...
    throw new Error(`TranquilLyrics [RetrievePackage] ${error?.message ?? "An Error Occured"}`);
  }
};
