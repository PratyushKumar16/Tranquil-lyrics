...
const log = {
  info: (...args: unknown[]) => {
    if (storage.get("developerMode") === "true") {
      console.log("[Tranquil Lyrics] [Query]", ...args);
    }
  },
  warn: (...args: unknown[]) => {
    if (storage.get("developerMode") === "true") {
      console.warn("[Tranquil Lyrics] [Query]", ...args);
    }
  },
  error: (...args: unknown[]) => {
    console.error("[Tranquil Lyrics] [Query]", ...args);
  },
};
...
export async function Query(
  ...
  const clientVersion = Session.TranquilLyrics.GetCurrentVersion();
