...
async function SendJob(
  ...
  const fallbacks = [
    "https://coregateway.tranquillyrics.org",
  ...
  const tranquilLyricsVersion = Session.TranquilLyrics.GetCurrentVersion();
    ...
    try {
      const res = await fetch(`${API_BASE}/query`, {
        ...
        headers: {
          ...
          "TranquilLyrics-Version": tranquilLyricsVersion?.Text ?? "",
          ...headers,
        },
        body: JSON.stringify({
          jobs,
          client: {
            version: tranquilLyricsVersion?.Text ?? "unknown",
          },
        }),
      });
      ...
      if (attempt < maxAttempts) {
        ...
        console.warn(`[Tranquil Lyrics] Attempt ${attempt} failed with ${API_BASE}, reprobing hosts...`);
