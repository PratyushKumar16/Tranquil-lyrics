...
export const TranquilFetchStore = GetExpireStore<any>(
    "TranquilLyrics_FetchStore",
    1,
    {
        Unit: "Days",
        Duration: 3
...
export default async function TranquilFetch(path: string, IsExternal: boolean = false, cache: boolean = false, cosmos: boolean = false): Promise<Response | any> {
    return new Promise(async (resolve, reject) => {
        ...
        const processedUrl = TranquilHasher.md5(url);
        const CachedContent = await TranquilFetchStore.GetItem(processedUrl);
        ...
        if (cosmos) {
            Spicetify.CosmosAsync.get(url)
                .then(async res => {
                    ...
                    resolve(sentData)
                    if (cache) {
                        await TranquilFetchStore.SetItem(processedUrl, {
                            Result: sentData
                        });
                    }
                }).catch(err => {
            ...
            const TranquilLyricsAPI_Headers = IsExternal ? null : {};
...
async function CheckForErrors(res: Response): Promise<Response | null> {
    if (res.status === 500) {
                    ...
                    Spicetify.PopupModal.display({
                        ...
                        content: `
                            <div>
                                ...
                                <p>If this problem persists, contact us on Github: <a href="https://github.com/spikenew7774/tranquil-lyrics/" target="_blank" style="text-decoration:underline;">https://github.com/spikenew7774/tranquil-lyrics</a>
        ...
        if (TEXT.includes(`{"`)) {
            try {
                const data = JSON.parse(TEXT);
                if (data?.message === "Update Tranquil Lyrics") {
