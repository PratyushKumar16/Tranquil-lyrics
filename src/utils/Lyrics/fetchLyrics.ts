...
export const LyricsStore = GetExpireStore<any>("TranquilLyrics_LyricsStore", 12, {
  Unit: "Days",
  Duration: 3,
}, isDev as true);

export default async function fetchLyrics(uri: string): Promise<[object | string, number] | null> {
  const IsTranquilRenderer = Defaults.LyricsRenderer === "Tranquil";
  ...
  try {
    ...
    if (!lyricsQuery) {
      console.error("[Tranquil Lyrics] Lyrics query not found");
      HideLoaderContainer();
      storage.set("currentlyFetching", "false");
      return ["lyrics-not-found", 404];
    }
    ...
    IsTranquilRenderer ? await ProcessLyrics(lyrics) : null;
