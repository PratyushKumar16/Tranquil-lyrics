...
function handleUserScroll(ScrollSimplebar: any | null) {
  ...
  if (!isUserScrolling) {
    ...
    if (lyricsContent) {
      ...
      console.warn(
        "TranquilLyrics: Could not find .LyricsContent in handleUserScroll to add HideLineBlur."
      );
      // --- END NEW ---
    }
  }
    ...
    if (currentLine) {
      ...
      if (timeSinceLastScroll > USER_SCROLL_COOLDOWN && isLineInViewport) {
        ...
        if (lyricsContent) {
          ...
          console.warn(
            "TranquilLyrics: Could not find .LyricsContent in ScrollToActiveLine to remove HideLineBlur."
          );
        }
        //}
...
export function CleanupScrollEvents() {
  ...
  scrolledToLastLine = false;
  scrolledToFirstLine = false;
  //console.log("TranquilLyrics scroll events cleaned up."); // Optional log
}
// --- END NEW ---
