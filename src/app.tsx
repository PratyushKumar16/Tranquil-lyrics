...
import "./css/DynamicBG/tranquil-dynamic-bg.css";
...
async function main() {
  ...
  if (!storage.get("lyricsRenderer")) {
    storage.set("lyricsRenderer", "Tranquil");
  }
  ...
  Defaults.TranquilLyricsVersion = window._tranquil_lyrics_metadata?.LoadedVersion ?? ProjectVersion;
  window._tranquil_lyrics_metadata = {}

  /* if (storage.get("lyrics_spacing")) {
    if (storage.get("lyrics_spacing") === "None") {
      document.querySelector("html").style.setProperty("--TranquilLyrics-LineSpacing", "0");
    }
    if (storage.get("lyrics_spacing") === "Small") {
      document.querySelector("html").style.setProperty("--TranquilLyrics-LineSpacing", "0.5cqw 0");
    }
    if (storage.get("lyrics_spacing") === "Medium") {
      document.querySelector("html").style.setProperty("--TranquilLyrics-LineSpacing", "1cqw 0");
    }
    if (storage.get("lyrics_spacing") === "Large") {
      document.querySelector("html").style.setProperty("--TranquilLyrics-LineSpacing", "1.5cqw 0");
    }
    if (storage.get("lyrics_spacing") === "Extra Large") {
      document.querySelector("html").style.setProperty("--TranquilLyrics-LineSpacing", "2cqw 0");
    }
  } */
  ...
  skeletonStyle.innerHTML = `
        ...
        /* This is a part of Tranquil Lyrics */
        @keyframes skeleton {
            to {
        ...
        @keyframes TranquilLoader_FadeIn {
          0% {
            opacity: 0;
          }
          100% {
        ...
        @keyframes TranquilLoader_FadeOut {
          0% {
            opacity: 1;
          }
          100% {
  ...
  skeletonStyle.id = "tranquilLyrics-additionalStyling";
        ...
        Button: new SpotifyPlayer.Playbar.Button(
          ...
          (self) => {
            if (!self.active) {
              ...
              Session.Navigate({ pathname: "/TranquilLyrics" });
              if (Global.Saves.shift_key_pressed) {
                const pageWhentil = Whentil.When(
                  () => document.querySelector<HTMLElement>(".Root__main-view #TranquilLyricsPage"),
                  () => {
  ...
  // console.log("[Tranquil Lyrics Debug] Setting up initial sidebar status check");
  ...
  if (!isTranquilSidebarMode && getQueueContainer()) {
    // console.log("[Tranquil Lyrics Debug] Got now playing view parent container");
    const sidebarStatus = storage.get("sidebar-status") ?? "";
    // console.log("[Tranquil Lyrics Debug] Sidebar status from storage:", sidebarStatus);
    if (sidebarStatus === "open") {
      // console.log("[Tranquil Lyrics Debug] Sidebar status is 'open', checking current path");
      if (Spicetify.Platform.History.location.pathname === "/TranquilLyrics") {
        // console.log("[Tranquil Lyrics Debug] Currently on /TranquilLyrics, going back");
        Session.GoBack();
        // console.log("[Tranquil Lyrics Debug] Setting up Whentil to open sidebar after navigation");
        Whentil.When(
          () =>
            !PageView.IsOpened && Spicetify.Platform.History.location.pathname !== "/TranquilLyrics",
          () => {
            // console.log("[Tranquil Lyrics Debug] Page closed and navigated away, opening sidebar");
            OpenSidebarLyrics(!!getQueueContainer());
          }
        );
      } else {
        // console.log("[Tranquil Lyrics Debug] Not on /TranquilLyrics, setting up Whentil to open sidebar");
        Whentil.When(
          () =>
            !PageView.IsOpened && Spicetify.Platform.History.location.pathname !== "/TranquilLyrics",
          () => {
            // console.log("[Tranquil Lyrics Debug] Conditions met, opening sidebar");
            OpenSidebarLyrics(!!getQueueContainer());
          }
        );
      }
...

  {
    ...
    fullscreenButton.element.id = "TranquilLyrics_FullscreenButton";

    const popupLyricsButton = ButtonList[2].Button;
    if (popupLyricsButton) {
      popupLyricsButton.element.style.order = "100000";
      popupLyricsButton.element.id = "TranquilLyrics_PopupLyricsButton";
    }
    ...
    const hideUnwantedButtons = (container: Element) => {
      for (const element of container.children) {
        ...
        if (
          ...
          element.id !== "TranquilLyrics_FullscreenButton" &&
          element.id !== "TranquilLyrics_PopupLyricsButton"
        ) {
  ...
  const Hometinue = async () => {
    ...
    if (fromVersion !== Defaults.TranquilLyricsVersion) {
      const div = document.createElement("div");
      const reactRoot = ReactDOM.createRoot(div);
      reactRoot.render(
        <UpdateDialog fromVersion={fromVersion} tranquilLyricsVersion={Defaults.TranquilLyricsVersion} />
      );

      PopupModal.display({
        title: "Tranquil Lyrics Updated!",
        content: div,
        isLarge: true,
        onClose: () => {
          reactRoot.unmount();
    ...
    storage.set("fromVersion", Defaults.TranquilLyricsVersion);

    // Lets set out Dynamic Background (tranquil-dynamic-bg) to the now playing bar
    ...
    const CleanupNowBarDynamicBgLets = () => {
      ...
      nowPlayingBar?.querySelector<HTMLElement>(".tranquil-dynamic-bg")?.remove();
      nowPlayingBar?.classList.remove("tranquil-dynamic-bg-in-this");
    ...
    async function applyDynamicBackgroundToNowPlayingBar(coverUrl: string | undefined) {
      ...
      try {
        if (!nowPlayingBar || cinemaViewExists || isTranquilSidebarMode) {
          lastImgUrl = null;
          CleanupNowBarDynamicBgLets();
          return;
        }
        ...
        nowPlayingBar.classList.add("tranquil-dynamic-bg-in-this");
    ...
    async function loadPage(location: Location) {
      if (location.pathname === "/TranquilLyrics") {
        if (isTranquilSidebarMode) {
          CloseSidebarLyrics();
        }
        Whentil.When(
        ...
        if (lastLocation?.pathname === "/TranquilLyrics") {
          PageView.Destroy();
          if (!button) return;
    ...
    if (button) {
      button.Button.tippy.setContent("Tranquil Lyrics");
    }
    ...
    button.Button.tippy.setProps({
      ...
      onMount(instance: any) {
          ...
          if (!PageView.IsTippyCapable) {
            ...
            instance.setContent("Tranquil Lyrics");
    ...
    // Events
    {
      ...
      Global.Event.listen("session:navigation", (data: Location) => {
        if (data.pathname === "/TranquilLyrics/Update") {
          storage.set("fromVersion", Defaults.TranquilLyricsVersion);
          window._tranquil_lyrics_metadata = {};
          Session.GoBack();
