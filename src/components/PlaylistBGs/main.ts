...
if (!ThisPageRoot) {
    ...
    Global.Event.listen("session:navigation", (data: NavigationData) => {
        if (Session.GetPreviousLocation()?.pathname.startsWith("/playlist") && ThisPageRoot.classList.contains("tranquil-playlist-bg")) {
            const underMainView = ThisPageRoot.querySelector<HTMLElement>(".under-main-view");
            if (underMainView) {
                underMainView.innerHTML = ``;
                ThisPageRoot.classList.remove("tranquil-playlist-bg");
            }
        }
        ...
        if (data.pathname.startsWith("/playlist")) {
            ...
            //const OriginalImageUrl = `https://storage.tranquil-lyrics.spikerko.org/playlist-bgs/${currentPlaylistId}.png`;
            ...
            const PROD_HOSTNAME = "https://portal.tranquillyrics.org";
            ...
            ThisPageRoot.classList.add("tranquil-playlist-bg");
...
/* async function ExtractColorFromImage(imageUrl: string) {
    const req = await fetch(`https://tranquilcolor.spikerko.org/extract?url=${imageUrl}`);
