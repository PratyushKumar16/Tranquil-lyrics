...
async function OpenPage(
  ...
  elem.id = "TranquilLyricsPage";

  if (Defaults.LyricsRenderer === "Tranquil") {
    elem.classList.add("TranquilRenderer");
  }
  ...
  const SkipTranquilFont = storage.get("skip-tranquil-font");
  if (SkipTranquilFont !== "true") {
    elem.classList.add("UseTranquilFont");
  }
  ...
  /* const ArtworkButton = document.querySelector<HTMLElement>("#TranquilLyricsPage .ContentBox .NowBar .Header .Artwork");
...
function AppendViewControls(ReAppend: boolean = false) {
  ...
  elem.innerHTML = `
        ...
        ${
          Defaults.LyricsRenderer === "Tranquil"
      ...
      try {
        ...
        compactModeToggle.addEventListener("click", () => {
          ...
          if (Fullscreen.IsOpen || Fullscreen.CinemaViewOpen) {
            if (IsCompactMode()) {
              TranquilLyricsPage?.classList.remove("ForcedCompactMode");
              DisableCompactMode();
              storage.set("ForceCompactMode", "false");
            } else {
              TranquilLyricsPage?.classList.add("ForcedCompactMode");
              EnableCompactMode();
      ...
      try {
        ...
        fullscreenBtn.addEventListener("click", async () => {
          ...
          if (Fullscreen.CinemaViewOpen) {
            ...
            await EnterTranquilLyricsFullscreen();
      ...
      try {
        ...
        devToolsButton.addEventListener("click", () => {
          ...
          Spicetify.PopupModal.display({
            title: "Tranquil Lyrics DevTools",
            isLarge: true,
            content: `
                            <div class="TranquilLyricsDevToolsContainer">
                                <div class="Setting">
                                    <div class="SettingName"><span>Load TTML (for the current song)</span></div>
                                    <div class="SettingValue">
                                        <button onclick="window._tranquil_lyrics.execute('upload-ttml')">Load TTML</button>
                                    </div>
                                </div>
                                <div class="Setting">
                                    <div class="SettingName"><span>Reset TTML (for the current song)</span></div>
                                    <div class="SettingValue">
                                        <button onclick="window._tranquil_lyrics.execute('reset-ttml')">Reset TTML</button>
                                    </div>
                                </div>
                            </div>
                        `,
...
interface TranquilLyricsNotificationReturnObject {
  cleanup: () => void;
  close: () => void;
  open: () => void;
}
...
export function TranquilLyrics_Notification({
  icon,
  ...
  closeBtn?: boolean;
}): TranquilLyricsNotificationReturnObject {
...
/* function SocketStatusChange(status: boolean) {
    if (!PageView.IsOpened) return;
    if (!document.querySelector("#TranquilLyricsPage")) return;
    const notif = TranquilLyrics_Notification({
        icon: Icons.LyricsPage,
        metadata: {
            title: "Connection Error",
            description: "We're recconecting you back to Tranquil Lyrics. Be patient."
        },
        type: "Warning",
        closeBtn: false
    })
