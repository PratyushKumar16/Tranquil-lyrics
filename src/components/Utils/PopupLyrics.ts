...
import { isTranquilSidebarMode, CloseSidebarLyrics } from "./SidebarLyrics.ts"
...
export const OpenPopupLyrics = async () => {
  ...
  // Copy the main TranquilLyrics style element
  // Find any <style> element in the DOM that includes '#TranquilLyricsPage' in its textContent
  // Find all <style> elements in the DOM that include '#TranquilLyricsPage' in their textContent
  const tranquilLyricsStyleElement = document.querySelector("#slstyles");
  let tranquilLyricsStyleContent: string | null = null;

  if (tranquilLyricsStyleElement) {
    if (tranquilLyricsStyleElement.tagName.toLowerCase() === "link") {
      // @ts-ignore
      const href = tranquilLyricsStyleElement.getAttribute("href");
      if (href) {
        try {
          const res = await fetch(href);
          if (res.ok) {
            tranquilLyricsStyleContent = await res.text();
          }
        } catch (e) {
          tranquilLyricsStyleContent = null;
        }
      }
    } else if (tranquilLyricsStyleElement.tagName.toLowerCase() === "style") {
      tranquilLyricsStyleContent = tranquilLyricsStyleElement.textContent;
    }
  }

  if (tranquilLyricsStyleContent) {
    const newStyleElement = document.createElement("style");
    newStyleElement.textContent = tranquilLyricsStyleContent;
    currentPipWindow.document.head.appendChild(newStyleElement);
  }

  // Additionally, copy the styles element with the id 'tranquilLyrics-additionalStyling'
  const additionalStyling = document.getElementById("tranquilLyrics-additionalStyling");
  if (additionalStyling) {
    const newAdditionalStyling = document.createElement("style");
    newAdditionalStyling.id = "tranquilLyrics-additionalStyling";
  ...
  currentPipWindow.document.body.innerHTML = `<div class="app-drag-region"></div><div class="tranquil-pip-wrapper"></div>`;

  const pipWrapper = currentPipWindow.document.body.querySelector(".tranquil-pip-wrapper") as HTMLElement;
