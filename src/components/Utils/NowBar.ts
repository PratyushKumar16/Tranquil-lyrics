...
import { isTranquilSidebarMode } from "./SidebarLyrics.ts";
...
function ApplyMarquee(baseWidth, elementWidth, name) {
    ...
    `;
    style.id = `tranquil-lyrics-marquee_${name}`;
...
function OpenNowBar(skipSaving: boolean = false) {
  ...
  if (!NowBar) return;
  const tranquilLyricsPage = PageContainer;
  if (isTranquilSidebarMode) {
    tranquilLyricsPage?.classList.add("NowBarStatus__Closed");
    tranquilLyricsPage?.classList.remove("NowBarStatus__Open");
    return;
  }
  UpdateNowBar(true);
  NowBar.classList.add("Active");

  if (tranquilLyricsPage) {
    tranquilLyricsPage.classList.remove("NowBarStatus__Closed");
    tranquilLyricsPage.classList.add("NowBarStatus__Open");
  }
  ...
  /* const DragBox = Fullscreen.IsOpen
        ? document.querySelector(
              "#TranquilLyricsPage .ContentBox .NowBar .Header .MediaBox .MediaContent"
          )
        : document.querySelector(
              "#TranquilLyricsPage .ContentBox .NowBar .Header .MediaBox .MediaImageContainer"
          ); */

  /* {
        const dropZones = document.querySelectorAll(
            "#TranquilLyricsPage .ContentBox .DropZone"
        );
        ...
        DragBox.addEventListener("dragstart", (e) => {
            ...
            document.querySelector("#TranquilLyricsPage").classList.add("SomethingDragging");
        ...
        DragBox.addEventListener("dragend", () => {
            const missingLyrics = storage.get("currentLyricsData")?.toString() === `NO_LYRICS:${SpotifyPlayer.GetSongId()}`;
            if (missingLyrics) return;
            document.querySelector("#TranquilLyricsPage").classList.remove("SomethingDragging");
        ...
        dropZones.forEach((zone) => {
            ...
            zone.addEventListener("drop", (e) => {
                ...
                document.querySelector("#TranquilLyricsPage").classList.remove("NowBarSide__Left");
                document.querySelector("#TranquilLyricsPage").classList.remove("NowBarSide__Right");
                document.querySelector("#TranquilLyricsPage").classList.add(`NowBarSide__${newClass.replace("Side", "")}`);
...
/* function UpdateNowBar(force = false) {
    const NowBar = document.querySelector("#TranquilLyricsPage .ContentBox .NowBar");
    if (!NowBar) return;
...
function NowBar_SwapSides() {
  ...
  if (CurrentSide === "left") {
    ...
    tranquilLyricsPage.classList.remove("NowBarSide__Left");
    tranquilLyricsPage.classList.add("NowBarSide__Right");
  } else if (CurrentSide === "right") {
    storage.set("NowBarSide", "left");
    NowBar.classList.remove("RightSide");
    NowBar.classList.add("LeftSide");
    tranquilLyricsPage.classList.remove("NowBarSide__Right");
    tranquilLyricsPage.classList.add("NowBarSide__Left");
  } else {
    storage.set("NowBarSide", "right");
    NowBar.classList.remove("LeftSide");
    NowBar.classList.add("RightSide");
...
function DeregisterNowBarBtn() {
  /* const nowBarButton = document.querySelector(
        "#TranquilLyricsPage .ContentBox .ViewControls #NowBarToggle"
    );
    nowBarButton?.remove(); */
  PageView.AppendViewControls(true);
}
