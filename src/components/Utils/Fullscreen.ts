...
export const EnterTranquilLyricsFullscreen = async () => {
...
function Open(skipDocumentFullscreen: boolean = false, moveElement: boolean = true) {
  const TranquilPage = PageContainer;
  ...
  if (TranquilPage) {
    ...
    // Handle DOM changes
    if (moveElement) TransferElement(TranquilPage, Root);
    TranquilPage.classList.add("Fullscreen");
    ...
    const NowBarToggle = TranquilPage.querySelector<HTMLElement>(
    ...
    const MediaBox = TranquilPage.querySelector<HTMLElement>(
      ".ContentBox .NowBar .Header .MediaBox"
    );
    const MediaImageContainer = TranquilPage.querySelector<HTMLElement>(
    ...
    if (MediaBox && MediaImageContainer) {
      ...
      if (TranquilPage) {
        TranquilPage.addEventListener("mouseenter", Page_MouseIn, { signal });
        TranquilPage.addEventListener("mousemove", Page_MouseMove, { signal });
        TranquilPage.addEventListener("mouseleave", Page_MouseOut, { signal });
      }
    }
  ...
  setTimeout(() => {
    ...
    if (storage.get("ForceCompactMode") === "true" && !IsCompactMode()) {
      TranquilPage?.classList.add("ForcedCompactMode");
      EnableCompactMode();
    }
  }, 750);
...
function Close(isPip: boolean = false) {
  ...
  if (TranquilPage) {
    ...
    // Handle DOM changes
    if (!isPip) TransferElement(TranquilPage, GetPageRoot() as HTMLElement);
    TranquilPage.classList.remove("Fullscreen");
