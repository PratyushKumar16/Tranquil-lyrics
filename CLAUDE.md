...
Build outputs: `dist/` (intermediate), `builds/tranquil-lyrics.mjs` (final bundle consumed by Spicetify).

Version and project name are managed in `project/config.ts` — bump this file when releasing.

`isDev` flag in `src/components/Global/Defaults.ts` switches the API base URL between `http://localhost:3000` and `https://api.tranquillyrics.org`.
...
Two hot keys (`currentlyFetching`, `currentLyricsData`) are kept in module-level variables for perf; everything else is stored in `Spicetify.LocalStorage` under the `TranquilLyrics-` prefix.

**Global singleton** (`src/components/Global/Global.ts`)
Thin wrapper around `window._tranquil_lyrics` for cross-module scope sharing and a custom event bus (`EventManager`).
...
- `window._tranquil_lyrics` — extension's own global scope, managed through `Global.SetScope` / `Global.GetScope`.
